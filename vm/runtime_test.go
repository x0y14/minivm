...
func TestBrainfuck_In_Brainfuck(t *testing.T) {
	// ref: https://github.com/canoon/bfbf/blob/master/bf.bf
	src := `
[-]+>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>[-]<<<<<<<<<<<<<<<<<<<<<<<
<<<<<<<<<<<<<<<<<<<<<<<<<<<[>[-],>[-]+++++++++++++++++++++++++++++++++>>>[-]>>[-
]<<<<<<[->>>>+>>+<<<<<<]>>>>>>[-<<<<<<+>>>>>>]<[-]>[-]<<<<<[->>>>+>+<<<<<]>>>>>[
...
[-]>[-]<<[->+>+<<]>>[-<<+>>]<[<<<<<<<[-]>>>>>>>[-]]<<<<<<<]`
	...
	prog := compileBF(src, 64*1024)

	cfg := &Config{StackSize: 1 << 15, HeapSize: 1 << 17}
	rt := NewRuntime(prog, cfg)

	// 「helloworld」を出力するBFプログラム
	helloBF := strings.ReplaceAll(`
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
++++++++++++++++++++++++++++++++++++++++.---.+++++++..+++.++++++
++.--------.+++.------.--------.
`, "\n", "")

	rt.stdin = bytes.NewBufferString(helloBF + "!")
